...
const int  stimTTL = 12;    // water IR sensor input
const int  stimTTLintan = 13;    // water IR sensor input
const int piezoPin = 8;           //Specify digital pin on the Arduino that the positive lead of piezo buzzer is attached.
const unsigned long NoseHoldingInterval = 800; // number of millisecs to hold
const unsigned long stimDurationOff = 200; // number of millisecs after nosePoke
const unsigned long stimDurationOn = 600; // number of millisecs after nosePoke
...
void loop()
{
  ...
  if ((CurrentNoseInfraState == LOW) && (PreviousNoseInfraState == HIGH))  // detect nose poke (it breaks the IR beam -> goes from Hi->Lo)
  {
    ...
    if (EnableNosePoke == true)
    {
      ...
      startStimFlag = true; //flag that stimulation is in progress
      //Serial.println(startTime);
      //Serial.println("Nose Poke detected!");
    }
  }
  
  if ((currentMillis - startMillis) >= stimDurationOff) //if the stim wait period has elapsed
  {
    if ((startStimFlag == true) && (EnableStimulation == true)) //nose poke is taking place
    {
      if (CurrentNoseInfraState == LOW)  //if the nose is currently in
      {
        startStimFlag == false;
        digitalWrite(stimTTL, HIGH); //send ttl to RF signal generator
        digitalWrite(stimTTLintan, HIGH); //send ttl to RF signal generator
        startStimMillis = currentMillis;
        EnableStimulation = !EnableStimulation;
      }
    }
  }
  
   if ((currentMillis - startStimMillis) >= stimDurationOn) //if the stim wait period has elapsed
  {
        startStimFlag == true;
        digitalWrite(stimTTL, LOW); //send ttl to RF signal generator
        digitalWrite(stimTTLintan, LOW); //send ttl to RF signal generator
  }
  ...
  if ((CurrentWaterInfraState == LOW) && (PreviousWaterInfraState == HIGH))
  {
    if (EnableWater == true)
    {
      ...
      resetBEDPins();
      CorrectPerformance = trials / NosePoke * 100;
