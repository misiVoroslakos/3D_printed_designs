...
const unsigned long NoseHoldingInterval = 2000; // number of millisecs to hold
const unsigned long stimDurationOff = 500; // number of millisecs after nosePoke
const unsigned long stimDurationOn = 1000; // number of millisecs to stimulate
...
void loop()
{
  ...
  if ((CurrentNoseInfraState == LOW) && (PreviousNoseInfraState == HIGH))  // detect nose poke (it breaks the IR beam -> goes from Hi->Lo)
  {
   ...
   if ((millis() - startMillis) >= stimDurationOff) //if the nosepoke is happening and the stimOff period has elapsed
  {
    if (EnableStimulation == true) 
    {
      if (CurrentNoseInfraState == LOW)  //if the nose is currently in 
      {
        ...
        stimTTLstate = !stimTTLstate;
        startStimMillis = currentMillisStimStart;  //save the time when the stimulation started
        digitalWrite(stimTTL,stimTTLstate); //send ttl to RF signal generator
      }
    }
  }
  
  if ((millis() - startStimMillis) >= stimDurationOn) //if the stimOn period has elapsed
  {
    if (EnableStopStimulation == true)
    {
      stimTTLstate = !stimTTLstate;
      digitalWrite(stimTTL,stimTTLstate);
      EnableStopStimulation = !EnableStopStimulation;
    }
  }
