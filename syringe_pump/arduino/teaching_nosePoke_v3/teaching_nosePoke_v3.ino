...
const unsigned long NoseHoldingInterval = 3000; // number of millisecs to hold
...
int NoseInfraState = HIGH;
int WaterInfraState = HIGH;
int startFlag = 0;
//boolean NosePoke = false;
boolean EnableWater = false;
boolean EnableNosePoke = true;
unsigned long startTime;  // this is a global
...
void loop() {
  ...
  //Serial.println(EnableWater);
  //Serial.println(WaterInfraState);
  //Serial.println(NoseInfraState);
  
  if (NoseInfraState == LOW) { // when nose breaks the IR beam
    startTime = millis();
    previousTime = startTime;
    //Serial.println(startTime);
    Serial.println("Nose Poke detected!");
  }
  if ((millis() - startTime) <= NoseHoldingInterval) {  //still waiting it out
    if (digitalRead(NoseInfraPin) == 1) { // nose moved out
      startFlag = 0;
    }
  }
  ...
  if (WaterInfraState == LOW && EnableWater == true) {
