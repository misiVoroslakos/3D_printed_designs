// This script is for controlling the on and off of the heating peltier for the cooling box behavioral experiment.
...
// Other variables
const long heat_switch_interval = 20 * minute;         // interval at which to blink (milliseconds)
int isNewLoop = 1;
int nLoopsRemained = 2;//number of loops in total to iterate through the 4 spots
int buttonState = 0;
...
void loop() {
  ...
  if (isInLoop == 1 && nLoopsRemained > 0) {
    ...
    if (currentMillis - previousMillis_heat_loop >= heat_switch_interval) {
      digitalWrite(relay_01, LOW);
      digitalWrite(IntanTTL_01, LOW);
      //digitalWrite(relay_02, LOW);
      //digitalWrite(IntanTTL_02, LOW);
      ...
      if (currentMillis - previousMillis_heat_loop >= 2*heat_switch_interval) {
